...
private func hidInputReport(
  ...
  guard let sender else {
    MainActor.assumeIsolated {
      monitor.reportCallbackRejected(
        reason: "missing_sender", result: result, reportLength: reportLength)
    }
    return
  }
  guard reportLength > 0 else {
    MainActor.assumeIsolated {
      monitor.reportCallbackRejected(
        reason: "empty_report", result: result, reportLength: reportLength)
    }
    return
  }
...
final class HIDRemoteMonitor {
  ...
  var onButtonPressed:
    ((UUID?, String, RemoteButton) -> (profileID: UUID, shouldPerformAction: Bool)?)?
  ...
  nonisolated static var inputMonitoringAccess: IOHIDAccessType {
    IOHIDCheckAccess(kIOHIDRequestTypeListenEvent)
  }

  nonisolated static var isInputMonitoringGranted: Bool {
  ...
  private func activateDevice(
    ...
    guard monitorResult == kIOReturnSuccess || allowManagerFallback else {
      updateStatus(
        LocalizedMessage(
          "button_mapping.error.device_read_failed", arguments: [String(monitorResult)]))
      AppLogger.shared.write(
  ...
  private func process(usages: Set<UInt16>) {
    ...
    for usage in pressed.sorted() {
      ...
      let preflightRecognizesDoubleClick =
        settings.configuredAction(
          for: button,
          trigger: .doubleClick,
          profileID: preflightProfileID
        ).action != .disabled
      let preflightRecognizesLongPress =
        settings.configuredAction(
          for: button,
          trigger: .longPress,
          profileID: preflightProfileID
      ...
      let usesNativePassthrough =
        preflightProfileID != nil
        && shouldUseNativePassthrough(
          button: button,
      ...
      let recognizesDoubleClick =
        settings.configuredAction(
          for: button,
          trigger: .doubleClick,
          profileID: profileID
        ).action != .disabled
      let recognizesLongPress =
        settings.configuredAction(
          for: button,
          trigger: .longPress,
          profileID: profileID
  ...
  static func resolvedFingerprintForReport(
    ...
    case .accepted(let fingerprint): fingerprint
    case .rejected: nil
    }
  }
  ...
  nonisolated static func isKarabinerElementsInstalled(
  ...
  private func shouldUseNativePassthrough(
    ...
    return (button == .left && action == .arrowLeft) || (button == .right && action == .arrowRight)
  }
  ...
  private func processGestureCommands(
    ...
    for command in commands {
      switch command {
      case .scheduleDoubleClickTimeout(let button):
        scheduleDoubleClickTimeout(for: button)
      case .cancelDoubleClickTimeout(let button):
        doubleClickTimers.removeValue(forKey: button)?.cancel()
      case .scheduleLongPressTimeout(let button):
        scheduleLongPressTimeout(for: button)
      case .cancelLongPressTimeout(let button):
        longPressTimers.removeValue(forKey: button)?.cancel()
      case .trigger(let button, let trigger):
        diagnosticLogger(
  ...
  private func resetGestureRecognition() {
    for timer in doubleClickTimers.values {
      timer.cancel()
    }
  ...
  private func resetInputState() {
    ...
    for timer in nonRepeatableReleaseTimers.values {
      timer.cancel()
    }
  ...
  static func fingerprint(for device: IOHIDDevice) -> String? {
    ...
    guard
      let location = IOHIDDeviceGetProperty(device, kIOHIDLocationIDKey as CFString) as? NSNumber
    else {
      return nil
    }
