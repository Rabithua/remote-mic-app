...
enum VirtualAudioConnectionLifecyclePolicy {
  static func shouldBeActive(
    ...
    if bluetoothVoiceActive || testToneActive {
      return true
    }
  ...
  func configure(deviceUID: String) -> Bool {
    ...
    guard !deviceUID.isEmpty else {
      ...
      AppLogger.shared.write(
        "AUDIO CONFIGURE skipped reason=no_selected_device previous={\(previousState)}")
      return false
    }
    ...
    guard let outputUnit = engine.outputNode.audioUnit else {
      ...
      AppLogger.shared.write(
        "AUDIO CONFIGURE failed reason=no_output_unit target={\(CoreAudioDeviceCatalog.deviceDiagnostic(device))}"
      )
      return false
    }
    var deviceID = device.id
    ...
    do {
      ...
      AppLogger.shared.write(
        "AUDIO READY target={\(CoreAudioDeviceCatalog.deviceDiagnostic(device))} state={\(diagnosticState())}"
      )
      return true
    } catch {
  ...
  func playTestTone(completion: @escaping (Bool) -> Void) -> Bool {
    guard isReadyForTestTone,
      ...
      let buffer = makeBuffer(
        samples: TestToneGenerator.samples(sampleRate: sourceFormat.sampleRate))
    else { return false }
