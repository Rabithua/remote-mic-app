...
enum RemoteVoiceFunctionMappingPolicy {
  ...
  static func applying(
    ...
    var desired =
      existing.filter {
        $0.source != remoteVoiceKey.source && $0.source != suppressedRemotePowerKey.source
      } + [voiceMapping]
    if let powerMapping {
...
final class RemoteVoiceFunctionMapper {
  ...
  func apply(
    ...
    let fullySuppressedLocations = Set(
      matchedCountsByLocation.compactMap { locationID, count in
    ...
    let suppressionScope =
      isPowerKeySuppressed
  ...
  private func rollback(
    ...
    for registryID in newlyStoredRegistryIDs.subtracting(registryIDsNeedingRestore) {
