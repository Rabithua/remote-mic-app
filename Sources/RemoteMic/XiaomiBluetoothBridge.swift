...
protocol XiaomiBluetoothBridgeDelegate: AnyObject {
  ...
  func bluetoothBridge(
    _ bridge: XiaomiBluetoothBridge, didIdentifyRemoteModel model: XiaomiRemoteModel)
  func bluetoothBridge(
    _ bridge: XiaomiBluetoothBridge, didUpdatePowerState state: RemotePowerState?)
}
...
final class XiaomiBluetoothBridge: NSObject {
  ...
  private func write(_ data: Data) -> Bool {
    ...
    let type: CBCharacteristicWriteType =
      transmitCharacteristic.properties.contains(.writeWithoutResponse)
      ? .withoutResponse
      : .withResponse
...
extension XiaomiBluetoothBridge: @preconcurrency CBCentralManagerDelegate {
  ...
  func centralManager(
    ...
    guard let generation = centralGeneration,
      ...
      serviceMatch || isCandidate(peripheral)
...
extension XiaomiBluetoothBridge {
  ...
  fileprivate func handleDiscoveredCharacteristics(
    ...
    let isOptionalService =
      service.uuid == batteryServiceUUID || service.uuid == deviceInformationServiceUUID
    guard shouldRun,
      isCurrent(peripheral),
      currentGeneration() == generation,
      isOptionalService
    ...
    if service.uuid == deviceInformationServiceUUID {
      ...
      guard
        let characteristic = service.characteristics?.first(where: { $0.uuid == modelNumberUUID })
      else {
        return
      }
      peripheral.readValue(for: characteristic)
  ...
  fileprivate func handleNotificationState(
    ...
    let isOptionalCharacteristic =
      characteristic.uuid == batteryLevelUUID || characteristic.uuid == batteryLevelStatusUUID
  ...
  fileprivate func handleCharacteristicValue(
    ...
    if characteristic.uuid == modelNumberUUID {
      ...
      let normalizedModelNumber = modelNumber.trimmingCharacters(in: .whitespacesAndNewlines)
        .uppercased()
